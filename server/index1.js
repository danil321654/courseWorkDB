...
app.get('/techtonica/apprentices', async (req, res) => {
    ...
    const contactsTable = await client.query('SELECT * FROM "Drug"');
...
app.get('/techtonica/cohorts', async (req, res) => {
  const client = await pool.connect();
  const contactsTable = await client.query('SELECT * FROM "Category"');
  res.json(contactsTable.rows);
  client.release();
...
app.get('/techtonica/apprentices/:id', async (req, res) =>{
  const client = await pool.connect();
  const eventsTable = await client.query('SELECT * FROM products WHERE id = $1', [req.params.id]);
